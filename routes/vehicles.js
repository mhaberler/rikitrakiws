...
module.exports = function(router, db) {
    ...
    router.get('/v1/vehicles/number', function(req, res) {
        logger.info('get number of vehicles');
        db.collection('vehicles', function(err, collection) {
            ...
            collection.find(req.query).count(function(err, count) {
                logger.info('number of vehicles is...', count);
                if (err) {
    ...
    router.post('/v1/vehicles', isValidToken, function(req, res) {
        logger.info('add vehicle', req.body);
        ...
        if (v(req.body)) {
            ...
            vehicle.blob = Buffer.from(req.body.blob, 'base64');
    ...
    // retrieve vehicle info, including blob if ?blob=x is given
    router.get('/v1/vehicles/:vehicleId', function(req, res) {
        var vehicleId = req.params.vehicleId;
        logger.info('retrieve vehicle: ' + vehicleId);
        db.collection('vehicles', function(err, collection) {
            collection.findOne({
                'vehicleId': vehicleId
            }, {
